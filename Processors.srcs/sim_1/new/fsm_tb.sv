...
module fsm_tb;
    ...
    initial begin
        ...
        // Regression check for a fixed bug: BB1 used to test `bbcase != 2'b00`
        // (i.e. "G_nonzero || G_lessthanzero") for BOTH BRNE and BRLT, so BRLT
        // branched on any nonzero G instead of only negative ones. BB1 now
        // re-checks opcode[0] to pick the right flag per instruction.
        check_branch(OP_BRLT, 1'b1, 1'b0, 1'b0, "BRLT not taken when G>0");
