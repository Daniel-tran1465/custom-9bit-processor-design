...
case (current_state)
        ...
        SUB_S3: begin
		  ...
		  BB1: begin
				// IRout still holds the branch opcode here (IR only reloads at
				// FETCH_WAIT), so re-check opcode[0] to tell BRNE (110, taken on
				// brne) apart from BRLT (111, taken on brlt) instead of taking
				// either flag being set (which made BRLT alias BRNE).
				if (opcode[0] ? brlt : brne) begin
					next_state = BB2;
				end else begin
					Done = 1'b1;
					next_state = FETCH;
